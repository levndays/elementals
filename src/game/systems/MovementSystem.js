...
export class MovementSystem {
            ...
            _handleJump(player) {
                if (!player.input.jumpRequested) return;

                if (player.isSwimming) {
                    player.physics.body.velocity.y = GAME_CONFIG.PLAYER.JUMP_HEIGHT / 2; // Swim up
                    return;
                }
            ...
            _handleSlam(player) {
                if (player.isSwimming) {
                    if (player.input.slamRequested) {
                        player.physics.body.velocity.y = -GAME_CONFIG.PLAYER.JUMP_HEIGHT / 2; // Swim down
                    }
                    return;
                }
